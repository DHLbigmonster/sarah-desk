    ...
    return new Promise((resolve, reject) => {
      ...
      this.ws.on('open', () => {
        ...
        const requestConfig: Record<string, unknown> = {
          ...
          enable_itn: true,
          enable_ddc: true,
          enable_nonstream: this.config.enableNonstream ?? true,
          show_utterances: true,
          result_type: 'full',
        };

        const corpus: Record<string, string> = {};
        if (this.config.boostingTableId) {
          corpus.boosting_table_id = this.config.boostingTableId;
        } else if (this.config.boostingTableName) {
          corpus.boosting_table_name = this.config.boostingTableName;
        }

        if (Object.keys(corpus).length > 0) {
          requestConfig.corpus = corpus;
        }

        if (this.config.correctTableId) {
          requestConfig.correct_table_id = this.config.correctTableId;
        } else if (this.config.correctTableName) {
          requestConfig.correct_table_name = this.config.correctTableName;
        }
      ...
      this.ws.on('unexpected-response', (_request, response) => {
        ...
        response.on('end', () => {
          logger.error('Response body', { body, logId });
          if (this.closedByClient) {
            logger.warn('Handshake failed after client-side disconnect', {
              statusCode: response.statusCode,
              logId,
            });
            return;
  ...
  private cleanup(): void {
    if (this.ws) {
      ...
      ws.on('error', (error: Error) => {
        logger.debug('Ignored WebSocket error after cleanup', { error: error.message });
      });
      ws.on('unexpected-response', (_request, response) => {
        logger.debug('Ignored unexpected response after cleanup', {
          statusCode: response.statusCode,
